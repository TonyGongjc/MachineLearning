...
class Perceptron(object):
    def __init__(self, rate = 0.01):
        train = Perceptron.read_file('bclass/bclass/bclass-train')
        test = Perceptron.read_file('bclass/bclass/bclass-test')
    ...
    def update(self, numIter = 50, norm = 2):
        weights = np.ones((self.trainx_col,1))
...
if __name__ == "__main__":
    perceptron =Perceptron(rate=0.01)
    perceptron.update_average(numIter= 500, norm=2)
    print(min(perceptron.train_error),min(perceptron.test_error))
    plt.plot(perceptron.train_error,'r--',perceptron.test_error,'b--')
    plt.show()
